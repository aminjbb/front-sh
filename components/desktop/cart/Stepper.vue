<template>
  <div class="stepper">
    <header class="stepper__header d-flex align-center w-100">
      <div v-for="(step, index) in steps" :key="`step${index}`" class="d-flex align-center cur-p"
           :class="index + 1 < steps.length ? 'flex-grow-1' : ''" @click="previousStep(index+1)">
        <div class="stepper__header__content">
          <v-badge :color="active[index+1] ? 'primary': previousSteps[index+1] ? 'grey-darken-3' : 'grey'"
                   :content="index+1" class="ml-2 mr-2" @click="activeStep = index+1 " inline/>
          <span class="t15 w400 pl-2"
                :class="active[index+1] ? 'text-primary': previousSteps[index+1] ? 'text-grey-darken-3' : 'text-grey'">{{
              step
            }}</span>
        </div>
      ...
      <v-row v-if="activeStep !== 4">
        <v-col md="9">
          ...
          <template v-if="activeStep === 2">
            ...
            <generalNotification class="mb-3" borderColorCode="E91E63" color="pink" icon="mdi-alert-outline"
                                 text="کاربران عزیز، تمام سفارشات ثبت شده در بازه 1402/12/23 الی 1403/01/14، به ترتیب اولویت از 15 فروردین ارسال خواهد شد."/>
            <desktopCartSendingInformationTime v-if="orderAddressId" :sendingMethods="sendingMethods"
                                               @selectedDate="getTime" @selectedWay="getWay"/>
          </template>
        ...
        <v-col md="3">
          ...
          <v-card class="py-5 px-3">
            ...
            <div v-if="!freeDelivery && data.sending_price !== 0 && data.sending_price !== null"
            ...
            <v-btn @click="nextStep()" height="44" :title="buttonText[activeStep - 1]"
