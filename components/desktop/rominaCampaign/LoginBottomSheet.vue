      ...
      <div class="voucher-auth voucher-auth--mobile bg-white pb-6 h-100 d-flex flex-column w-100">
        <div class="d-flex align-center justify-space-between py-6 px-6">
                <span class="t16 w600 text-sGray">
                    <v-icon v-if="showAddAddress === true" icon="mdi-chevron-right" color="text-sGray"
                            style="font-size: 31px;" @click="showAddressList"/>
                    دریافت هدیه
                </span>
        ...
        <div v-if="!logined" class="px-5 my-3">
                    ...
                    <v-form ref="phoneNumberForm" class="w-100">
                      <div class="w-100 form-inner">
                        <div class="mt-6 w-100">
                          ...
                          <v-text-field class="campaign-auth__input" v-model="mobile" :rules="mobileRule" type="tel"
                                        variant="outlined" placeholder="شماره موبایل" height="46px"/>
                        </div>
                      </div>
                    </v-form>
                  </template>
          ...
          <div cols="12" class="d-flex align-center justify-center flex-column mb-5">
            <img data-not-lazy v-if="loginStep === 1" src="~/assets/images/campaign/romina-login1.svg" width="82"
                 height="80"/>
            <img data-not-lazy v-if="loginStep === 2" src="~/assets/images/campaign/romina-otp.svg" width="82"
                 height="80"/>
          </div>
        </div>
        ...
        <div v-else class="px-2">
          <template v-if="noOrder">
            <div class="d-flex flex-column align-center justify-center w100 h100 px-4">
              <p class="t16 w300 l32 text-sGray text-center">
                با شماره‌ی شما هنوز سفارشی در شاواز ثبت نشده ولی می‌‌‌‌‌‌‌‌تونی بعد از ثبت اولین سفارشت به این صفحه سر
                بزنی و هدیه‌‌‌‌‌‌‌‌ رومینا رو دریافت کنی.
              </p>

              <img data-not-lazy class="mt-10 mb-13" src="~/assets/images/campaign/romina-login3.svg" width="120"
                   height="105"/>

              <v-btn href="/" :loading="loading" color="primary" block type="submit" height="46px"
                     class="campaign-auth__btn mb-5">
          ...
          <template v-if="hasOrder">
            <template v-if="showAddress">
              ...
              <div v-if="userAddress && userAddress.length" class="campaign-address px-4"
                   :class="{'height-auto' :showMore}">
                <div v-for="(address , index) in userAddress.slice(0,showMoreCount)" :key="`address${index}`"
                     class="d-flex align-center campaign-address__item" @click="selectAddress(address.id)"
                     :id="`address${address.id}`" :class="index === 0 ? 'activeAddress' : ''">
              ...
              <div v-if="userAddress.length > 3" class="w-100 d-flex justify-end px-4">
                            <span v-if="showMore === false"
                                  class="text-primary d-flex align-center t12 w700 mb-5 mt-2 cur-p"
                                  @click="activeShowMore">
                                مشاهده بیشتر
                                <v-icon icon="mdi-chevron-down" color="primary"></v-icon>
                            </span>

                <span v-if="showMore === true" class="text-primary d-flex align-center t12 w700 mb-5 mt-2 cur-p"
                      @click="deActiveShowMore">
                                بستن
              ...
              <div class="px-4">
                <v-btn @click="getGift()" :loading="createLoading" color="primary" block type="submit" height="46px"
                       class="campaign-auth__btn">
              ...
              <v-form v-model="valid" ref="addAddress" class="px-4">
                <v-row>
                  <v-col cols="12" md="6" class="py-2">
                    <v-autocomplete v-if="provinceList" :items="provinceList" density="compact" variant="outlined"
                                    :rules="rule" label="استان *" v-model="form.province"
                                    @update:modelValue="getCitiesList()"/>
                  </v-col>

                  <v-col cols="12" md="6" class="py-2">
                    <v-autocomplete :items="cityList" density="compact" variant="outlined" :rules="rule" label="شهر *"
                                    v-model="form.city" @update:modelValue="getCityName()"/>
                  </v-col>

                  <v-col cols="12" md="4" class="py-2">
                    <v-text-field density="compact" variant="outlined" label="کد پستی *" :rules="postalCodeRule"
                                  :append-inner-icon="rules ? 'mdi-check' : ''" v-model="form.postal_code" height="42"/>
                  </v-col>

                  <v-col cols="12" md="4" class="py-2">
                    <v-text-field density="compact" variant="outlined" label="پلاک *" :rules="rule"
                                  :append-inner-icon="rules ? 'mdi-check' : ''" v-model="form.number" height="42"/>
                  </v-col>

                  <v-col cols="12" md="4" class="py-2">
                    <v-text-field density="compact" variant="outlined" label="واحد" :rules="rule"
                                  :append-inner-icon="rules ? 'mdi-check' : ''" v-model="form.room_number" height="42"/>
                  </v-col>

                  <v-col cols="12" class="py-2">
                    <v-text-field density="compact" variant="outlined" label="تکمیل آدرس *" hide-details :rules="rule"
                                  :append-inner-icon="rules ? 'mdi-check' : ''" v-model="form.address"/>
                  </v-col>
                </v-row>
          ...
          <template v-if="lastStep === true">
            <div class="d-flex flex-column align-center justify-center">
              <img data-not-lazy class="mt-8 mb-8" src="~/assets/images/campaign/romina-last-step.svg" width="140"
                   height="130"/>
        ...
        <div v-if="!logined" class="px-6">
          <template v-if="loginStep === 1">
            <v-btn @click="sendOTP()" :loading="loading" color="primary" block type="submit" height="46px"
                   class="campaign-auth__btn">
          ...
          <template v-else-if="loginStep === 2">
            <v-btn @click="verifyOTP()" :loading="loading" color="primary" block rounded="xl" type="submit"
                   height="48px" class="campaign-auth__btn br8">
