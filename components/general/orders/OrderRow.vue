<template>
  <section v-if="content" class="order-row pt-5">
    <header class="d-flex justify-space-between align-center mb-8">
      <div class="order-row__details d-flex align-center flex-grow-1">
        <div class="d-flex align-center flex-grow-1">
          ...
          <span v-if="content?.status"
                class="t12 w400 text-pink-darken-2 mr-1">{{ findOrderStatus(content?.status) }}</span>
        </div>
        ...
        <div class="d-flex align-center flex-grow-1">
          ...
          <span v-if="content?.submit_date_fa"
                class="t12 w400 text-pink-darken-2 number-font">{{ content?.submit_date_fa }}</span>
        </div>
        ...
        <div class="d-flex align-center flex-grow-1">
          ...
          <span v-if="content?.order_number"
                class="t12 w400 text-pink-darken-2 number-font">{{ content?.order_number }}</span>
        </div>
    ...
    <div class="d-flex justify-end px-3 pb-3">
      <template v-if="content && content.status == 'payment_in_progress'">
        <v-btn @click="repeatPayment()" height="28" style="height: 32px !important;" title="پرداخت مجدد"
      ...
      <template v-if="content && content.status == 'payment_out_date'">
        <v-btn @click="createOrder()" :loading="reCreateOrderLoading" height="28" style="height: 32px !important;"
               title="سفارش مجدد" class="btn btn--submit">
          سفارش مجدد
        </v-btn>
      </template>
    </div>
  ...
  <generalOrdersPaymentMethodModal ref="selectPaymentMethod" :view="screenType === 'desktop' ? 'desktop' : 'mobile'"
                                   :orderId="content?.id" :paidPrice="content?.paid_price"/>
</template>
