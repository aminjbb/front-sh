          ...
          <v-form @submit.prevent="sendOTP" ref="phoneNumberForm" class="w-100">
            <div class="w-100 form-inner">
              ...
              <div v-if="showCancel" class="mt-7 d-flex align-center justify-space-between">
                <v-btn :disabled="activeButton === true ? false :true" :loading="loading" color="primary" type="submit"
                       height="46px" class="game-auth__btn ">
                  دریافت کد
                </v-btn>
    ...
    async verifyOTP() {
      try {
        ...
        if (response.status === 200) {
          ...
          if (!this.showSuccess) {
            this.$store.commit('set_snackBar', {
              show: true, text: response.data.messag, status: 'success'
            })
          }
