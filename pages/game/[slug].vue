  ...
  <client-only>
    <main class="v-game">
      ...
      <v-container class="parent-width">
        <v-row v-if="screenType === 'desktop'" class="h-100">
          ...
          <v-col cols="12" class="d-flex justify-center v-game__lucky-wheel">
            <div class="d-flex flex-column align-center" v-if="luckyWheel.length">
              <generalGameFortuneWheel :token="token" ref="generalGameFortuneWheel" @updateForm="updateList"
                                       @luckyBtnClicked="openLoginModal" :items="luckyWheel" :isLogin="isLogin"
                                       :limit="turnPerUser" :userUsed="voucherList?.length"/>
            </div>
          </v-col>
        ...
        <generalGameLoginModal ref="modalLoginRef" @logined="logined"
                               :view="screenType === 'desktop' ? 'desktop' : 'mobile'"/>
...
export default {
  ...
  mounted() {
    ...
    if (this.userToken){
      ...
      this.getUserVoucherList(this.userToken);
