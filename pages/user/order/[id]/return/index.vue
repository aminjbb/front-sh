      ...
      <v-row>
        ...
        <div class="col-9 pa-4 mobile-pa-0">
            ...
            <div v-if="returnStep === 1" class="px-5">
              <template v-for="(order, index) in userOrder?.details" :key="`order${order.id}`">
                ...
                <div v-if="value[index] !== false && value[index] && chooseAll !== true"
                     class="v-order--return__accordion">
                  <div class="mb-5">
                    <label class="d-block t13 text-grey-darken-1 mb-2">علت مرجوعی<span
                        class="text-red-accent-4">*</span> </label>
                    <v-select
                        density="compact"
                        variant="outlined"
                        single-line
                  ...
                  <div class="mb-5">
                    <label class="d-block t13 text-grey-darken-1 mb-2">روش بازگشت وجه<span
                        class="text-red-accent-4">*</span> </label>
                    <v-select
                        density="compact"
                        variant="outlined"
          ...
          <div v-if="returnStep === 2 || returnStep === 1">
            <generalWarning
                text="ثبت درخواست مرجوعی به معنی تایید درخواست نیست. در صورت تایید درخواست مرجوعی، از طریق پیامک به شما اطلاع خواهیم داد."/>
          </div>
        </div>
      </v-row>
    </v-container>
...
export default {
  ...
  setup() {
    ...
    const {getOrder, order, returnOrRejectOrder, orderReturnOrRejectObject} = new Order()
    useHead({
      title,
      meta: [{
        name: 'description',
    ...
    return {getOrder, order, returnOrRejectOrder, orderReturnOrRejectObject}
  },
